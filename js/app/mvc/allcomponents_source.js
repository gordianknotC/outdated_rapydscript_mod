...
function modifyClass(name,prop){
		if($.browser.msie | $.browser.safari){
			var atstyle = $('body style#dynamic');
			if(atstyle.length >0){
				...
				for(i=0;i<csses.length;i++){
					...
					if(css.indexOf(name)!= -1){
						...
						if($.browser.msie) $('body style#dynamic')[0].styleSheet.cssText = csses.join("}")
						if($.browser.safari) $('body style#dynamic')[0].innerHTML = csses.join("}")
	        			//$('body').html(_html);
						
						return
					}
				}
				if($.browser.msie) $('body style#dynamic')[0].styleSheet.cssText = csses.join("}")+name+'{'+prop +'}'
				if($.browser.safari) $('body style#dynamic')[0].innerHTML = csses.join("}")+name+'{'+prop +'}'
				return
			}
			
			$('<style id="dynamic" type="text/css">'+name+'{'+prop +'}'+'</style>').appendTo('body')
			
		}else{
			var atstyle = $('head style');
			var csses = atstyle.html().trim().split('}');
			prop = 'background:#f00;'
			name = '#navbt div:hover'
			for(i=0;i<csses.length;i++){
				var css = csses[i];
				if(css.indexOf(name)!= -1){
						var targetcss = css.split("{");
