...
jamal.fn.extend({
	 ...
	 */
    session: function(){
		jamal.session.loaded = true
		console.log('this.config.py.session = ',this.config.session)
		console.log('this.config.py.session == true?? ',this.config.session == true)
        if(this.config.session) {
            this.log('Session activated');
            this.session.active = true;
            this.session.since = 0;
            
            jamal.ajaxSend(function(evt ,xhr ,setting){
